...
@Component({
  selector: 'app-modal-renewal-student',
  templateUrl: './modal-renewal-student.component.html',
  styleUrl: './modal-renewal-student.component.scss'
})
export class ModalRenewalStudentComponent implements OnChanges{
  @Input() student: any;
  ...
  updateRenewalChecklist(item: string): void {
    this.queueService.updateRenewalChecklist(this.student._id, item).subscribe(
      (response) => {
  ...
  scheduleRenewalInterview(): void {
    ...
    // Convert to Asia/Manila timezone, then to UTC
    ...
    this.queueService.scheduleRenewalInterview(this.student._id, isoSchedule, isoDeadline).subscribe(
      (response) => {
  ...
  rejectRenewalStudent(): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.queueService.rejectRenewalStudent(this.student._id).subscribe(
          (response) => {
            Swal.close(); 
            Swal.fire({
              icon: 'success',
          ...
          if (response.message === 'Student is already approved.') {
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                ...
                this.queueService.approveRenewalStudent(this.student.studentNo).subscribe(
                  (response) => {
                    Swal.close();
                    Swal.fire({
                      icon: 'success',
